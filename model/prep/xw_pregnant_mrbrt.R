...
prep_data <- function(df) {
  # read in data
  df <- df[sex == "Female" & age_start > 10 & age_end < 55 & sample_size >= 10]
  # prep data
...
save_matches <- function(df,me) {
  write.csv(df[,.(seq.ref, seq.alt, location_id, sex, ratio, ratio_se, year_mean)],
            file=paste0("FILEPATH/",me,"_matched_pregnant_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".csv"),
            row.names=F)
  print(paste0("Matches saved!"))
}
...
fit_mrbrt <- function(df, me, folder) {
  ...
  fit1 <- run_mr_brt(
    output_dir = paste0('FILEPATH/',me), # user home directory
...
apply_mrbrt <- function(df,me,folder){ 
  ...
  # write-out
  message(paste0("Crosswalk ratios applied! Writing out"))
  write.csv(data,paste0("FILEPATH/bundle_4754_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".csv"),row.names=F) 
  return(data)
  }
