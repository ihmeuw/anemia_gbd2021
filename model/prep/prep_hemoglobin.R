...
### Date: 06/29/2019
### Project: Anemia 
### Purpose: Primary data prep script for mean hemoglobin and anemia prevalence models
#####################################################################################
### ARGS
###
### me    -> "anemia" to run all MEs or individual me name ("hemoglobin",
###          "hemoglobin_pregnant","anemia_total","anemia_mod_sev","anemia_sev", 
###          "anemia_total_pregnant","anemia_mod_sev_pregnant","anemia_sev_pregnant")
###
### decomp_step -> GBD decomp step to pull from/upload to
###
### crosswalk_description -> description for crosswalk upload
#####################################################################################
...
prep_age_sex_split <- function(df, bundle_id){
  ...
  message(paste0("Data prepped for age and sex splitting! Writing out"))
  write.csv(data,paste0("FILEPATH/bundle_",bundle_id,"_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".csv"), row.names=F)
  return(data)
}
...
age_sex_split <- function(df, bundle_id, decomp_step){
  ...
  pdf(paste0("FILEPATH/bundle_",bundle_id,"_age_sex_split_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".pdf"))
  ...
  if (nrow(split) > 0){
    ...
    split <- merge(split, pops, by=c("location_id", "year_id", "sex_id", "age_group_id"), all.x=T)  ##sy: merge 'split' and 'pops' dfs
    split <- merge(split, asp, by=c("sex_id", "age_group_id"))                                      ##sy: merge 'split' and 'asp' (age sex pattern) dfs
    split[, pop_group := sum(population), by="split_id"]                                         ##sy: this sums all of the total pops by split_id, which should be age-sex-location-country specific
    ...
    ## Split
    split[, val := val * (pop_group/population) * (R/R_group) ]  # this multiplies the mean (val),
    # by the observed ratio of the sex-age population size to the whole population size for that country-year,
    # by the theoretical ratio of the sex-age population size to the whole population size for that country-year
  ...
  ## Save
  message(paste0("Data age- and sex-split! Writing out"))
  write.csv(split,paste0("FILEPATH/bundle_",bundle_id,"_",format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".csv"), row.names=F)
  return(split)
}
...
upload_xwalk_version <- function(df, bundle_id, bundle_version, description){
  ...
  FOLDER <- "imp_anemia"
  BUNDLE <- bundle_id
  path <- paste0("FILEPATH/crosswalk_bundle_",BUNDLE,"_",
                 format(lubridate::with_tz(Sys.time(), tzone="America/Los_Angeles"), "%Y-%m-%d"),".xlsx")
...
prep_hemoglobin <- function(me, decomp_step=NA, crosswalk_description=NA) {
  ...
  if (me=="anemia") bundle_ids <- c(4754,8108,8120,8117) # add pregnancy bundles
  if (me!="anemia"){
